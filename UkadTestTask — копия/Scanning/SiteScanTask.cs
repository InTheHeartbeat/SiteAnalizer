...
{    
    public class SiteScanTask : ISiteScanTask
    {
        ...
        {                     
            ...
            var result = ping.Send(new Uri(Site.Url).Host);
            ResultState.SiteResponseTime = new TimeSpan(0,0,0,0,(int) result.RoundtripTime);            
            ...
            foreach (Sitemap sitemap in Site.Sitemaps)
            {
                foreach (SitemapUrl notScannedUrl in sitemap.Urls.Where(u => !u.Completed))
                {
                    ...
                    State.TextState = "Scanning address " + notScannedUrl.Url + "(" + (State.ScannedAddresses + 1).ToString() + "/" +
            ...
            State.ProccessState = ScanProccessState.Completed;
            Site.Sitemaps.ForEach(sm => ResultState.PageLoadedMinTime = TimeSpan.FromMilliseconds(sm.Urls.Min(t => t.LoadTime.TotalMilliseconds)));
            Site.Sitemaps.ForEach(sm => ResultState.PageLoadedMaxTime = TimeSpan.FromMilliseconds(sm.Urls.Max(t => t.LoadTime.TotalMilliseconds)));
            Site.Sitemaps.ForEach(sm => ResultState.PageLoadedMiddleTime = TimeSpan.FromMilliseconds(sm.Urls.Average(t => t.LoadTime.TotalMilliseconds)));            
        }
        ...
        private void LoadSitemap()
        {
            ...
            Site.Sitemaps = (new SitemapProvider()).GetSitemapsFromUrl($"{Site.Url}/sitemap.xml");
        ...
        private SitemapUrl InterviewUrl(SitemapUrl url)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                try
                {
                    string result = client.GetStringAsync(url.Url).Result;
                    sw.Stop();
